...
public:
	...
	*/
	...
	void OP20(){ if (!(F & ZERO_FLAG)) JR(); else PC += 3; cycles = 8; }
	...
	void OP28(){ if ((F & ZERO_FLAG)) JR(); else PC += 3; cycles = 8; }
	...
	void OP2F(){ A = ~A; SETBIT(F, SUBTRACT_FLAG_BIT); SETBIT(F, HALF_CARRY_FLAG_BIT); ++PC; cycles = 4; }

	void OP30(){ if (!(F & CARRY_FLAG)) JR(); else PC += 3; cycles = 8; }
	...
	void OP37(){ SETBIT(F, HALF_CARRY_FLAG_BIT); CLRBIT(F, SUBTRACT_FLAG_BIT); CLRBIT(F, HALF_CARRY_FLAG_BIT); ++PC; cycles = 4; }
	void OP38(){ if ((F & CARRY_FLAG)) JR(); else PC += 3; cycles = 8; }
	...
	void OP3F(){ ((F & CARRY_FLAG) ? CLRBIT(F, CARRY_FLAG_BIT) : SETBIT(F, HALF_CARRY_FLAG_BIT)); ++PC; cycles = 4;
	...
	void OP98(){ SUB(A, B + ((F & CARRY_FLAG) >> CARRY_FLAG_BIT)); cycles = 4; } // SBC A, n
	...
	void OPC0(){ if (!(F & ZERO_FLAG)) RET(); else PC +=3; cycles = 8; }
	void OPC1(){ POP(B, C); ++PC; cycles = 12; }
	void OPC2(){ if( !(F & ZERO_FLAG) ) JP(); else PC += 3; cycles = 12; }
	void OPC3(){ JP(); cycles = 12; }
	void OPC4(){ if (!(F & ZERO_FLAG)) CALL(); else PC += 3; cycles = 12; }
	void OPC5(){ PUSH(B, C); ++PC; cycles = 16; }
	void OPC6(){ ADD(A, MEM[++PC]); cycles = 8; }
	void OPC7(){ RST( 0x00 ); cycles = 32; }
	void OPC8(){ if ((F & ZERO_FLAG)) RET(); else PC += 3; cycles = 8; }
	void OPC9(){ RET(); cycles = 8; }
	void OPCA(){ if ((F & ZERO_FLAG)) JP(); else PC += 3; cycles = 12; }
	void OPCB(){ cout << "CB Opcode called!" << endl; /* PREFIX CB OPCODES - DO NOT USE. */}
	void OPCC(){ if ((F & ZERO_FLAG)) CALL(); else PC += 3; cycles = 12; }
	void OPCD(){ CALL(); cycles = 12; } // CALL nn
	void OPCE(){ ADD(A, MEM[++PC] + ((F & CARRY_FLAG) >> CARRY_FLAG_BIT)); cycles = 8; }
	void OPCF(){ RST(0x08); cycles = 32; }

	void OPD0(){ if (!(F & CARRY_FLAG)) RET(); else PC += 3; cycles = 8; }
	void OPD1(){ POP(D, E); ++PC; cycles = 12; }
	void OPD2(){ if (!(F & CARRY_FLAG)) JP(); else PC += 3; cycles = 12; }
	void OPD3(){ /* DO NOTHING - BLANK OPCODE */ }
	void OPD4(){ if (!(F & CARRY_FLAG)) CALL(); else PC += 3; cycles = 12; }
	void OPD5(){ PUSH(D, E); ++PC; cycles = 16; }
	void OPD6(){ SUB(A, MEM[++PC]); cycles = 8; }
	void OPD7(){ RST(0x10); cycles = 32; }
	void OPD8(){ if ((F & CARRY_FLAG)) RET(); else PC += 3; cycles = 8; }
	void OPD9(){ RET(); MEM[INTERRUPT_ENABLE] = 0x01; cycles = 8;}
	void OPDA(){ if ((F & CARRY_FLAG)) JP(); else PC += 3; cycles = 12; }
	void OPDB(){ /* DO NOTHING - BLANK OPCODE */ }
	void OPDC(){ if ((F & CARRY_FLAG)) CALL(); else PC += 3; cycles = 12; }
	void OPDD(){ /* DO NOTHING - BLANK OPCODE */ }
	void OPDE(){ SUB(A, MEM[++PC] + ((F & CARRY_FLAG) >> CARRY_FLAG_BIT)); cycles = 8; }
