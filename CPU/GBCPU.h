...
public:
	...
	void execute();				// CPU Instruction Execution
                                ...
                                // Return Status register as one byte
    ...
    // Set Status register from a byte
    inline void SetF(byte F)
    {
	...
	*/
    ...
    void OP00(){ cout << "NOP" << endl; cycles = 4; ++PC; } // NOP
    void OP01(){ LD(C, MEM[++PC]); LD(B, MEM[++PC]); cycles = 12; ++PC; }
    void OP02(){ LD(MEM[CASTWD(B, C)], A); ++PC; cycles = 8; }
    void OP03(){ INC(B, C); cycles = 8; ++PC; }
    void OP04(){ INCR(B); cycles = 4; }
    void OP05(){ DECR(B); cycles = 4; }
    void OP06(){ LD(B, MEM[++PC]); ++PC; cycles = 8; }
    void OP07(){ RLCA(); cycles = 4; }
    void OP08(){ LD(MEM[CASTWD(MEM[++PC], MEM[++PC]) ], SP & 0xFF); ++PC; cycles = 20; }
    void OP09(){ ADD(CASTWD(B, C)); cycles = 8; }
    void OP0A(){ LD(A, MEM[CASTWD(B, C)]); ++PC; cycles = 8; }
    void OP0B(){ DEC(B, C); ++PC; cycles = 8; }
    void OP0C(){ INCR(C); cycles = 4; }
    void OP0D(){ DECR(C); cycles = 4; }
    void OP0E(){ LD(C, MEM[++PC]); ++PC; cycles = 8; }
    void OP0F(){ RRCA(); cycles = 4; }
    
    void OP10(){ ++PC; cycles = 4; cout << "HALT!" << endl;/* TODO: HALT CPU and LCD display until button pressed */ } // STOP
    void OP11(){ LD(E, MEM[++PC]); LD(D, MEM[++PC]); ++PC; cycles = 12; }
    void OP12(){ LD(MEM[CASTWD(D, E)], A); ++PC; cycles = 8; }
    void OP13(){ INC(D, E); ++PC; cycles = 8; }
    void OP14(){ INCR(D); cycles = 4; }
    void OP15(){ DECR(D); cycles = 4; }
    void OP16(){ LD(D, MEM[++PC]); ++PC; cycles = 8; }
    void OP17(){ RLA(); cycles = 4; }
    void OP18(){ JR(); cycles = 8; }
    void OP19(){ ADD(CASTWD(D, E)); cycles = 8; }
    void OP1A(){ LD(A, MEM[CASTWD(D, L)]); ++PC; cycles = 8; }
    void OP1B(){ DEC(D, E); ++PC; cycles = 8; }
    void OP1C(){ INCR(E); cycles = 4; }
    void OP1D(){ DECR(E); cycles = 4; }
    void OP1E(){ LD(E, MEM[++PC]); ++PC; cycles = 8; }
    void OP1F(){ RRA(); cycles = 4; }
    
    void OP20(){ if (ZERO_FLAG == false) JR(); else PC += 2; cycles = 8; }
    void OP21(){ LD(L, MEM[++PC]); LD(H, MEM[++PC]); ++PC; cycles = 12; } // TODO: Check if other bugs exists like in this one, where MSB was being loaded FIRST.
    void OP22(){ LD(MEM[CASTWD(H, L)], A); INC(H, L); ++PC; cycles = 8; }
    void OP23(){ INC(H, L); ++PC; cycles = 8; }
    void OP24(){ INCR(H); cycles = 4; }
    void OP25(){ DECR(H); cycles = 4; }
    void OP26(){ LD(H, MEM[++PC]); ++PC; cycles = 8; }
    void OP27(){ DAA(); cycles = 4; }
    void OP28(){ if (ZERO_FLAG == true) JR(); else PC += 2; cycles = 8; }
    void OP29(){ ADD(CASTWD(H, L)); cycles = 8; }
    void OP2A(){ LD(A, MEM[CASTWD(H, L)]); INC(H, L); ++PC; cycles = 8; }
    void OP2B(){ DEC(H, L); ++PC; cycles = 8; }
    void OP2C(){ INCR(L); cycles = 4; }
    void OP2D(){ DECR(L); cycles = 4; }
    void OP2E(){ LD(L, MEM[++PC]); ++PC; cycles = 8; }
    void OP2F(){ A = ~A; SUBTRACT_FLAG = true; HALF_CARRY_FLAG = true; ++PC; cycles = 4; }
    
    void OP30(){ if (CARRY_FLAG == false) JR(); else PC += 2; cycles = 8; }
    void OP31(){ byte lsb = MEM[++PC]; byte msb = MEM[++PC]; LD(SP, CASTWD(msb, lsb)); ++PC; cycles = 12; }
    void OP32(){ LD(MEM[CASTWD(H, L)], A); DEC(H, L); ++PC; cycles = 8; }
    void OP33(){ ++SP; cycles = 8; }
    void OP34(){ INCR(MEM[CASTWD(H, L)] ); cycles = 12; }
    void OP35(){ DECR(MEM[CASTWD(H, L)]); cycles = 12; }
    void OP36(){ LD(MEM[CASTWD(H, L)], MEM[++PC]); ++PC; cycles = 12; }
    void OP37(){ HALF_CARRY_FLAG = true; SUBTRACT_FLAG = false; HALF_CARRY_FLAG = false; ++PC; cycles = 4; }
    void OP38(){ if (CARRY_FLAG == true) JR(); else PC += 2; cycles = 8; }
    void OP39(){ ADD(SP); cycles = 8; }
    void OP3A(){ LD(A, MEM[CASTWD(H, L)]); DEC(H, L); ++PC; cycles = 8; }
    void OP3B(){ --SP; cycles = 8; }
    void OP3C(){ INCR(A); cycles = 4; }
    void OP3D(){ DECR(A); cycles = 4; }
    void OP3E(){ LD(A, MEM[++PC]); ++PC; cycles = 8; }
    void OP3F(){ (CARRY_FLAG == true ? CARRY_FLAG = false : HALF_CARRY_FLAG = true); ++PC; cycles = 4;
                 SUBTRACT_FLAG = true; HALF_CARRY_FLAG = true; }
    
    void OP40(){ LD(B, B); ++PC; cycles = 4; }
    void OP41(){ LD(B, C); ++PC; cycles = 4; }
    void OP42(){ LD(B, D); ++PC; cycles = 4; }
    void OP43(){ LD(B, E); ++PC; cycles = 4; }
    void OP44(){ LD(B, H); ++PC; cycles = 4; }
    void OP45(){ LD(B, L); ++PC; cycles = 4; }
    void OP46(){ LD(B, MEM[CASTWD(H, L)]); ++PC; cycles = 8; }
    void OP47(){ LD(B, A); ++PC; cycles = 4; }
    void OP48(){ LD(C, B); ++PC; cycles = 4; }
    void OP49(){ LD(C, C); ++PC; cycles = 4; }
    void OP4A(){ LD(C, D); ++PC; cycles = 4; }
    void OP4B(){ LD(C, E); ++PC; cycles = 4; }
    void OP4C(){ LD(C, H); ++PC; cycles = 4; }
    void OP4D(){ LD(C, L); ++PC; cycles = 4; }
    void OP4E(){ LD(C, MEM[CASTWD(H, L)]); ++PC; cycles = 8; }
    void OP4F(){ LD(C, A); ++PC; cycles = 4; }
    
    void OP50(){ LD(D, B); ++PC; cycles = 4; }
    void OP51(){ LD(D, C); ++PC; cycles = 4; }
    void OP52(){ LD(D, D); ++PC; cycles = 4; }
    void OP53(){ LD(D, E); ++PC; cycles = 4; }
    void OP54(){ LD(D, H); ++PC; cycles = 4; }
    void OP55(){ LD(D, L); ++PC; cycles = 4; }
    void OP56(){ LD(D, MEM[CASTWD(H, L)]); ++PC; cycles = 8; }
    void OP57(){ LD(D, A); ++PC; cycles = 4; }
    void OP58(){ LD(E, B); ++PC; cycles = 4; }
    void OP59(){ LD(E, C); ++PC; cycles = 4; }
    void OP5A(){ LD(E, D); ++PC; cycles = 4; }
    void OP5B(){ LD(E, E); ++PC; cycles = 4; }
    void OP5C(){ LD(E, H); ++PC; cycles = 4; }
    void OP5D(){ LD(E, L); ++PC; cycles = 4; }
    void OP5E(){ LD(E, MEM[CASTWD(H, L)]); cycles = 8; }
    void OP5F(){ LD(E, A); ++PC; cycles = 4; }
    
    void OP60(){ LD(H, B); ++PC; cycles = 4; }
    void OP61(){ LD(H, C); ++PC; cycles = 4; }
    void OP62(){ LD(H, D); ++PC; cycles = 4; }
    void OP63(){ LD(H, E); ++PC; cycles = 4; }
    void OP64(){ LD(H, H); ++PC; cycles = 4; }
    void OP65(){ LD(H, L); ++PC; cycles = 4; }
    void OP66(){ LD(H, MEM[CASTWD(H, L)]); ++PC; cycles = 8; }
    void OP67(){ LD(H, A); ++PC; cycles = 4; }
    void OP68(){ LD(L, B); ++PC; cycles = 4; }
    void OP69(){ LD(L, C); ++PC; cycles = 4; }
    void OP6A(){ LD(L, D); ++PC; cycles = 4; }
    void OP6B(){ LD(L, E); ++PC; cycles = 4; }
    void OP6C(){ LD(L, H); ++PC; cycles = 4; }
    void OP6D(){ LD(L, L); ++PC; cycles = 4; }
    void OP6E(){ LD(L, MEM[CASTWD(H, L)]); ++PC; cycles = 8; }
    void OP6F(){ LD(L, A); ++PC; cycles = 4; }
    
    void OP70(){ LD(MEM[CASTWD(H, L)], B); ++PC; cycles = 8; }
    void OP71(){ LD(MEM[CASTWD(H, L)], C); ++PC; cycles = 8; }
    void OP72(){ LD(MEM[CASTWD(H, L)], D); ++PC; cycles = 8; }
    void OP73(){ LD(MEM[CASTWD(H, L)], E); ++PC; cycles = 8; }
    void OP74(){ LD(MEM[CASTWD(H, L)], H); ++PC; cycles = 8; }
    void OP75(){ LD(MEM[CASTWD(H, L)], L); ++PC; cycles = 8; }
    void OP76(){ while( MEM[INTERRUPT_FLAG] == 0x00 ); ++PC; cycles = 4; } // HALT until an INTERRUPT occurs
    void OP77(){ LD(MEM[CASTWD(H, L)], A); ++PC; cycles = 8; }
    void OP78(){ LD(A, B); ++PC; cycles = 4; }
    void OP79(){ LD(A, C); ++PC; cycles = 4; }
    void OP7A(){ LD(A, D); ++PC; cycles = 4; }
    void OP7B(){ LD(A, E); ++PC; cycles = 4; }
    void OP7C(){ LD(A, H); ++PC; cycles = 4; }
    void OP7D(){ LD(A, L); ++PC; cycles = 4; }
    void OP7E(){ LD(A, MEM[CASTWD(H, L)]); ++PC; cycles = 8; }
    void OP7F(){ LD(A, A); ++PC; cycles = 4; }
    ...
    void OP88(){ ADD(A, B + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP89(){ ADD(A, C + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP8A(){ ADD(A, D + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP8B(){ ADD(A, E + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP8C(){ ADD(A, H + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP8D(){ ADD(A, L + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP8E(){ ADD(A, MEM[CASTWD(H,L)] + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 8; }
    void OP8F(){ ADD(A, A + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    ...
    void OP98(){ SUB(A, B + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; } // SBC A, n
    void OP99(){ SUB(A, C + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP9A(){ SUB(A, D + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP9B(){ SUB(A, E + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP9C(){ SUB(A, H + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP9D(){ SUB(A, L + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    void OP9E(){ SUB(A, MEM[CASTWD(H,L)] + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 8; }
    void OP9F(){ SUB(A, A + (CARRY_FLAG == true ? 0x01 : 0x00)); cycles = 4; }
    ...
    void OPC0(){ if (ZERO_FLAG == false) RET(); else ++PC; cycles = 8; }
    void OPC1(){ POP(B, C); ++PC; cycles = 12; }
    void OPC2(){ if( ZERO_FLAG == false ) JP(); else PC += 3; cycles = 12; }
    void OPC3(){ JP(); cycles = 12; }
    void OPC4(){ if (ZERO_FLAG == false) CALL(); else PC += 3; cycles = 12; }
    void OPC5(){ PUSH(B, C); ++PC; cycles = 16; }
    void OPC6(){ ADD(A, MEM[++PC]); cycles = 8; }
    void OPC7(){ RST( 0x00 ); cycles = 32; }
    void OPC8(){ if (ZERO_FLAG == true) RET(); else ++PC; cycles = 8; }
    void OPC9(){ RET(); cycles = 8; }
    void OPCA(){ if (ZERO_FLAG == true) JP(); else PC += 3; cycles = 12; }
    void OPCB(){ cout << "CB Opcode called!" << endl; /* PREFIX CB OPCODES - DO NOT USE. */}
    void OPCC(){ if (ZERO_FLAG == true) CALL(); else PC += 3; cycles = 12; }
    void OPCD(){ CALL(); cycles = 12; } // CALL nn
    void OPCE(){ ADD(A, MEM[++PC] + (CARRY_FLAG == true)); cycles = 8; }
    void OPCF(){ RST(0x08); cycles = 32; }
    
    void OPD0(){ if (CARRY_FLAG == false) RET(); else ++PC; cycles = 8; }
    void OPD1(){ POP(D, E); ++PC; cycles = 12; }
    void OPD2(){ if (CARRY_FLAG == false) JP(); else PC += 3; cycles = 12; }
    void OPD3(){ /* DO NOTHING - BLANK OPCODE */ }
    void OPD4(){ if (CARRY_FLAG == false) CALL(); else PC += 3; cycles = 12; }
    void OPD5(){ PUSH(D, E); ++PC; cycles = 16; }
    void OPD6(){ SUB(A, MEM[++PC]); cycles = 8; }
    void OPD7(){ RST(0x10); cycles = 32; }
    void OPD8(){ if (CARRY_FLAG == true) RET(); else ++PC; cycles = 8; }
    void OPD9(){ RET(); MEM[INTERRUPT_ENABLE] = 0x01; cycles = 8;}
    void OPDA(){ if (CARRY_FLAG == true) JP(); else PC += 3; cycles = 12; }
    void OPDB(){ /* DO NOTHING - BLANK OPCODE */ }
    void OPDC(){ if (CARRY_FLAG == true) CALL(); else PC += 3; cycles = 12; }
    ...
    void OPEA(){ LD(MEM[CASTWD(MEM[++PC], MEM[++PC])], A); ++PC; cycles = 16; }
    ...
    void OPF8(){ LD(H, (SP + MEM[++PC]) >> 8); LD(L, (SP + MEM[PC]) & 0xFF); ++PC; cycles = 12; 
                 SUBTRACT_FLAG = false; ZERO_FLAG = false; } // TODO: Status flag settings
    void OPF9(){ LD(SP, CASTWD(H, L)); ++PC; cycles = 8; }
    void OPFA(){ LD(A, MEM[CASTWD(MEM[++PC], MEM[++PC])]); ++PC; cycles = 16; }
    void OPFB(){ DI_Executed = false; ++PC; cycles = 4; } // EI
    ...
    // RES
    void CBOP80(){ CLRBIT(B, 0); ++PC; cycles = 8; }
    void CBOP81(){ CLRBIT(C, 0); ++PC; cycles = 8; }
    void CBOP82(){ CLRBIT(D, 0); ++PC; cycles = 8; }
    void CBOP83(){ CLRBIT(E, 0); ++PC; cycles = 8; }
    void CBOP84(){ CLRBIT(H, 0); ++PC; cycles = 8; }
    void CBOP85(){ CLRBIT(L, 0); ++PC; cycles = 8; }
    void CBOP86(){ CLRBIT(MEM[CASTWD(H,L)], 0); ++PC; cycles = 8; }
    void CBOP87(){ CLRBIT(A, 1); ++PC; cycles = 8; }
    void CBOP88(){ CLRBIT(B, 1); ++PC; cycles = 8; }
    void CBOP89(){ CLRBIT(C, 1); ++PC; cycles = 8; }
    void CBOP8A(){ CLRBIT(D, 1); ++PC; cycles = 8; }
    void CBOP8B(){ CLRBIT(E, 1); ++PC; cycles = 8; }
    void CBOP8C(){ CLRBIT(H, 1); ++PC; cycles = 8; }
    void CBOP8D(){ CLRBIT(L, 1); ++PC; cycles = 8; }
    void CBOP8E(){ CLRBIT(MEM[CASTWD(H,L)], 1); ++PC; cycles = 8; }
    void CBOP8F(){ CLRBIT(A, 1); ++PC; cycles = 8; }
    
    void CBOP90(){ CLRBIT(B, 2); ++PC; cycles = 8; }
    void CBOP91(){ CLRBIT(C, 2); ++PC; cycles = 8; }
    void CBOP92(){ CLRBIT(D, 2); ++PC; cycles = 8; }
    void CBOP93(){ CLRBIT(E, 2); ++PC; cycles = 8; }
    void CBOP94(){ CLRBIT(H, 2); ++PC; cycles = 8; }
    void CBOP95(){ CLRBIT(L, 2); ++PC; cycles = 8; }
    void CBOP96(){ CLRBIT(MEM[CASTWD(H, L)], 2); ++PC; cycles = 8; }
    void CBOP97(){ CLRBIT(A, 2); ++PC; cycles = 8; }
    void CBOP98(){ CLRBIT(B, 3); ++PC; cycles = 8; }
    void CBOP99(){ CLRBIT(C, 3); ++PC; cycles = 8; }
    void CBOP9A(){ CLRBIT(D, 3); ++PC; cycles = 8; }
    void CBOP9B(){ CLRBIT(E, 3); ++PC; cycles = 8; }
    void CBOP9C(){ CLRBIT(H, 3); ++PC; cycles = 8; }
    void CBOP9D(){ CLRBIT(L, 3); ++PC; cycles = 8; }
    void CBOP9E(){ CLRBIT(MEM[CASTWD(H, L)], 3); ++PC; cycles = 8; }
    void CBOP9F(){ CLRBIT(A, 3); ++PC; cycles = 8; }
    
    void CBOPA0(){ CLRBIT(B, 4); ++PC; cycles = 8; }
    void CBOPA1(){ CLRBIT(C, 4); ++PC; cycles = 8; }
    void CBOPA2(){ CLRBIT(D, 4); ++PC; cycles = 8; }
    void CBOPA3(){ CLRBIT(E, 4); ++PC; cycles = 8; }
    void CBOPA4(){ CLRBIT(H, 4); ++PC; cycles = 8; }
    void CBOPA5(){ CLRBIT(L, 4); ++PC; cycles = 8; }
    void CBOPA6(){ CLRBIT(MEM[CASTWD(H, L)], 4); ++PC; cycles = 8; }
    void CBOPA7(){ CLRBIT(A, 4); ++PC; cycles = 8; }
    void CBOPA8(){ CLRBIT(B, 5); ++PC; cycles = 8; }
    void CBOPA9(){ CLRBIT(C, 5); ++PC; cycles = 8; }
    void CBOPAA(){ CLRBIT(D, 5); ++PC; cycles = 8; }
    void CBOPAB(){ CLRBIT(E, 5); ++PC; cycles = 8; }
    void CBOPAC(){ CLRBIT(H, 5); ++PC; cycles = 8; }
    void CBOPAD(){ CLRBIT(L, 5); ++PC; cycles = 8; }
    void CBOPAE(){ CLRBIT(MEM[CASTWD(H, L)], 5); ++PC; cycles = 8; }
    void CBOPAF(){ CLRBIT(A, 5); ++PC; cycles = 8; }
    
    void CBOPB0(){ CLRBIT(B, 6); ++PC; cycles = 8; }
    void CBOPB1(){ CLRBIT(C, 6); ++PC; cycles = 8; }
    void CBOPB2(){ CLRBIT(D, 6); ++PC; cycles = 8; }
    void CBOPB3(){ CLRBIT(E, 6); ++PC; cycles = 8; }
    void CBOPB4(){ CLRBIT(H, 6); ++PC; cycles = 8; }
    void CBOPB5(){ CLRBIT(L, 6); ++PC; cycles = 8; }
    void CBOPB6(){ CLRBIT(MEM[CASTWD(H, L)], 6); ++PC; cycles = 8; }
    void CBOPB7(){ CLRBIT(A, 6); ++PC; cycles = 8; }
    void CBOPB8(){ CLRBIT(B, 7); ++PC; cycles = 8; }
    void CBOPB9(){ CLRBIT(C, 7); ++PC; cycles = 8; }
    void CBOPBA(){ CLRBIT(D, 7); ++PC; cycles = 8; }
    void CBOPBB(){ CLRBIT(E, 7); ++PC; cycles = 8; }
    void CBOPBC(){ CLRBIT(H, 7); ++PC; cycles = 8; }
    void CBOPBD(){ CLRBIT(L, 7); ++PC; cycles = 8; }
    void CBOPBE(){ CLRBIT(MEM[CASTWD(H, L)], 7); ++PC; cycles = 8; }
    void CBOPBF(){ CLRBIT(A, 7); ++PC; cycles = 8; }
    
    // SET
    void CBOPC0(){ SETBIT(B, 0); ++PC; cycles = 8; }
    void CBOPC1(){ SETBIT(C, 0); ++PC; cycles = 8; }
    void CBOPC2(){ SETBIT(D, 0); ++PC; cycles = 8; }
    void CBOPC3(){ SETBIT(E, 0); ++PC; cycles = 8; }
    void CBOPC4(){ SETBIT(H, 0); ++PC; cycles = 8; }
    void CBOPC5(){ SETBIT(L, 0); ++PC; cycles = 8; }
    void CBOPC6(){ SETBIT(MEM[CASTWD(H,L)], 0); ++PC; cycles = 8; }
    void CBOPC7(){ SETBIT(A, 1); ++PC; cycles = 8; }
    void CBOPC8(){ SETBIT(B, 1); ++PC; cycles = 8; }
    void CBOPC9(){ SETBIT(C, 1); ++PC; cycles = 8; }
    void CBOPCA(){ SETBIT(D, 1); ++PC; cycles = 8; }
    void CBOPCB(){ SETBIT(E, 1); ++PC; cycles = 8; }
    void CBOPCC(){ SETBIT(H, 1); ++PC; cycles = 8; }
    void CBOPCD(){ SETBIT(L, 1); ++PC; cycles = 8; }
    void CBOPCE(){ SETBIT(MEM[CASTWD(H,L)], 1); ++PC; cycles = 8; }
    void CBOPCF(){ SETBIT(A, 1); ++PC; cycles = 8; }
    
    void CBOPD0(){ SETBIT(B, 2); ++PC; cycles = 8; }
    void CBOPD1(){ SETBIT(C, 2); ++PC; cycles = 8; }
    void CBOPD2(){ SETBIT(D, 2); ++PC; cycles = 8; }
    void CBOPD3(){ SETBIT(E, 2); ++PC; cycles = 8; }
    void CBOPD4(){ SETBIT(H, 2); ++PC; cycles = 8; }
    void CBOPD5(){ SETBIT(L, 2); ++PC; cycles = 8; }
    void CBOPD6(){ SETBIT(MEM[CASTWD(H, L)], 2); ++PC; cycles = 8; }
    void CBOPD7(){ SETBIT(A, 2); ++PC; cycles = 8; }
    void CBOPD8(){ SETBIT(B, 3); ++PC; cycles = 8; }
    void CBOPD9(){ SETBIT(C, 3); ++PC; cycles = 8; }
    void CBOPDA(){ SETBIT(D, 3); ++PC; cycles = 8; }
    void CBOPDB(){ SETBIT(E, 3); ++PC; cycles = 8; }
    void CBOPDC(){ SETBIT(H, 3); ++PC; cycles = 8; }
    void CBOPDD(){ SETBIT(L, 3); ++PC; cycles = 8; }
    void CBOPDE(){ SETBIT(MEM[CASTWD(H, L)], 3); ++PC; cycles = 8; }
    void CBOPDF(){ SETBIT(A, 3); ++PC; cycles = 8; }
    
    void CBOPE0(){ SETBIT(B, 4); ++PC; cycles = 8; }
    void CBOPE1(){ SETBIT(C, 4); ++PC; cycles = 8; }
    void CBOPE2(){ SETBIT(D, 4); ++PC; cycles = 8; }
    void CBOPE3(){ SETBIT(E, 4); ++PC; cycles = 8; }
    void CBOPE4(){ SETBIT(H, 4); ++PC; cycles = 8; }
    void CBOPE5(){ SETBIT(L, 4); ++PC; cycles = 8; }
    void CBOPE6(){ SETBIT(MEM[CASTWD(H, L)], 4); ++PC; cycles = 8; }
    void CBOPE7(){ SETBIT(A, 4); ++PC; cycles = 8; }
    void CBOPE8(){ SETBIT(B, 5); ++PC; cycles = 8; }
    void CBOPE9(){ SETBIT(C, 5); ++PC; cycles = 8; }
    void CBOPEA(){ SETBIT(D, 5); ++PC; cycles = 8; }
    void CBOPEB(){ SETBIT(E, 5); ++PC; cycles = 8; }
    void CBOPEC(){ SETBIT(H, 5); ++PC; cycles = 8; }
    void CBOPED(){ SETBIT(L, 5); ++PC; cycles = 8; }
    void CBOPEE(){ SETBIT(MEM[CASTWD(H, L)], 5); ++PC; cycles = 8; }
    void CBOPEF(){ SETBIT(A, 5); ++PC; cycles = 8; }
    
    void CBOPF0(){ SETBIT(B, 6); ++PC; cycles = 8; }
    void CBOPF1(){ SETBIT(C, 6); ++PC; cycles = 8; }
    void CBOPF2(){ SETBIT(D, 6); ++PC; cycles = 8; }
    void CBOPF3(){ SETBIT(E, 6); ++PC; cycles = 8; }
    void CBOPF4(){ SETBIT(H, 6); ++PC; cycles = 8; }
    void CBOPF5(){ SETBIT(L, 6); ++PC; cycles = 8; }
    void CBOPF6(){ SETBIT(MEM[CASTWD(H, L)], 6); ++PC; cycles = 8; }
    void CBOPF7(){ SETBIT(A, 6); ++PC; cycles = 8; }
    void CBOPF8(){ SETBIT(B, 7); ++PC; cycles = 8; }
    void CBOPF9(){ SETBIT(C, 7); ++PC; cycles = 8; }
    void CBOPFA(){ SETBIT(D, 7); ++PC; cycles = 8; }
    void CBOPFB(){ SETBIT(E, 7); ++PC; cycles = 8; }
    void CBOPFC(){ SETBIT(H, 7); ++PC; cycles = 8; }
    void CBOPFD(){ SETBIT(L, 7); ++PC; cycles = 8; }
    void CBOPFE(){ SETBIT(MEM[CASTWD(H, L)], 7); ++PC; cycles = 8; }
    void CBOPFF(){ SETBIT(A, 7); ++PC; cycles = 8; }
};
